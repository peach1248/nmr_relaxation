...
class ParameterDockWidget(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.set_initial_check = QtGui.QCheckBox('set Initial Values', self)
        self.set_initial = QtGui.QPushButton('Input Initial Values')

        i_label = QtGui.QLabel("I =")
        self.i_combo = QtGui.QComboBox(self)
        for x in ["1/2", "3/2", "5/2", "7/2", "9/2"]:
            self.i_combo.addItem(x)
        i_box = QtGui.QHBoxLayout()
        i_box.addWidget(i_label)
        i_box.addWidget(self.i_combo)

        gamma_label = QtGui.QLabel("gamma (MHz/T)")
        self.gamma = QtGui.QLineEdit(self)
        gamma_box = QtGui.QHBoxLayout()
        gamma_box.addWidget(gamma_label)
        gamma_box.addWidget(self.gamma)

        h0_label = QtGui.QLabel("H0 (T)")
        self.h0 = QtGui.QLineEdit(self)
        h0_box = QtGui.QHBoxLayout()
        h0_box.addWidget(h0_label)
        h0_box.addWidget(self.h0)

        K_label = QtGui.QLabel("K (%)")
        self.K = QtGui.QLineEdit(self)
        K_box = QtGui.QHBoxLayout()
        K_box.addWidget(K_label)
        K_box.addWidget(self.K)

        nuQ_label = QtGui.QLabel("nuQ (MHz)")
        self.nuQ = QtGui.QLineEdit(self)
        nuQ_box = QtGui.QHBoxLayout()
        nuQ_box.addWidget(nuQ_label)
        nuQ_box.addWidget(self.nuQ)

        eta_label = QtGui.QLabel("eta")
        self.eta = QtGui.QLineEdit(self)
        eta_box = QtGui.QHBoxLayout()
        eta_box.addWidget(eta_label)
        eta_box.addWidget(self.eta)

        theta_label = QtGui.QLabel("theta (rad)")
        self.theta = QtGui.QLineEdit(self)
        theta_box = QtGui.QHBoxLayout()
        theta_box.addWidget(theta_label)
        theta_box.addWidget(self.theta)

        phi_label = QtGui.QLabel("phi (rad)")
        self.phi = QtGui.QLineEdit(self)
        phi_box = QtGui.QHBoxLayout()
        phi_box.addWidget(phi_label)
        phi_box.addWidget(self.phi)

        self.calculation_button = QtGui.QPushButton("Calculate")

        self.inputs_group = QtGui.QGroupBox("inputs")
        group_box = QtGui.QVBoxLayout()
        for x in [i_box, gamma_box, h0_box, K_box, nuQ_box, eta_box, theta_box, phi_box]:
...
class FunctionDockWidget(QtGui.QWidget):
    """緩和曲線を表示するWidget
    オブジェクト:
    ...
    def __init__(self, parent=None):
        ...
        function_label = QtGui.QLabel("Relaxation Curve")
        self.function = QtGui.QTextEdit(self)
...
class InitialParameterWidget(QtGui.QWidget):
    ...
    def __init__(self, n, parent=None):
        ...
        self.a_button = QtGui.QPushButton("center A", self)
        self.b_button = QtGui.QPushButton("center B", self)
        for button in [self.a_button, self.b_button]:
            hbox.addWidget(button)
            QtCore.QObject.connect(button, QtCore.SIGNAL("clicked()"), self.ab_button_pushed)
        ...
        self.initial_text = []
        for x in range(n):
            label = QtGui.QLabel("m = {}/2".format((n - 1) - x * 2))
            self.initial_text.append(QtGui.QLineEdit(self))
    ...
    def get_parameter(self, n):
        try:
            ...
            QtGui.QMessageBox.warning(self, "Message", u"Input Initial Values")
            return []

        if abs(np.sum(np.array(n0))) > 1.e-10:
            QtGui.QMessageBox.warning(self, "Message", u"Sum of Initial Values is not zero")
            return []
        return n0
...
class AdvSettings(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        vbox = QtGui.QVBoxLayout(self)
        vbox.setAlignment(QtCore.Qt.AlignLeft)
        label = QtGui.QLabel("threshold of coefficient of exponential")
        self.eps_c_text = QtGui.QLineEdit(self)
        box = QtGui.QHBoxLayout()
        box.addWidget(label)
        box.addWidget(self.eps_c_text)
        vbox.addLayout(box)
        self.eps_c_text.connect(self.eps_c_text, QtCore.SIGNAL("textChanged(QString)"), self.parameter_set)

        label = QtGui.QLabel("threshold of intensity of signal")
        self.int_lim_text = QtGui.QLineEdit(self)
        box = QtGui.QHBoxLayout()
        box.addWidget(label)
        box.addWidget(self.int_lim_text)
        vbox.addLayout(box)
        self.int_lim_text.connect(self.int_lim_text, QtCore.SIGNAL("textEdited(QString)"), self.parameter_set)
...
class UI(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        self.parameter_widget = QtGui.QDockWidget("Parameters", self)
        ...
        QtCore.QObject.connect(self.parameter_dock.i_combo, QtCore.SIGNAL("activated(int)"), self.input_initial)

        self.function_dock = FunctionDockWidget()
        self.function_widget = QtGui.QDockWidget("Relaxation Curve", self)
        ...
        # menu bar
        viewParameter = QtGui.QAction('Parameter', self)
        viewParameter.setShortcut('Ctrl+P')
        QtCore.QObject.connect(viewParameter, QtCore.SIGNAL("triggered()"), self.parameter_widget.show)

        viewFunction = QtGui.QAction('Function', self)
        viewFunction.setShortcut('Ctrl+F')
        QtCore.QObject.connect(viewFunction, QtCore.SIGNAL("triggered()"), self.function_widget.show)

        advSetting = QtGui.QAction('Setting more', self)
        QtCore.QObject.connect(advSetting, QtCore.SIGNAL("triggered()"), self.advset_widget.show)
    ...
    def calculation(self):
        ...
        try:
            ...
            QtGui.QMessageBox.warning(self, "Message", u"Sorry but initial parameters are not available in NQR case")
